			...
			public void onSelection(SelectionEvent<TreeItem> event) {
				...
				if (item.getUserObject() instanceof Unit && item.getChildCount() == 0) {
					...
					if (item.getUserObject() instanceof Company && item.getChildCount() == 0) {
						...
						System.out.println(item.getText() + " selected..");
						devTree.fetchDepartments((Company)item.getUserObject());
					}
					if (item.getUserObject() instanceof Department && item.getChildCount() == 0) {
						selectedDepartment = (Department)item.getUserObject();
						System.out.println(item.getText() + " selected..");
						devTree.fetchUnits((Department)item.getUserObject());
					}
