...
public class DeviceMaintenancePanel extends VerticalPanel {
	...
	public void createNewDeviceMaintenancePanel(Device device) {
		...
		HorizontalPanel headerPanel = AhoWidgets.createContentHeader(device.getId()+" " + device.getDeviceName() + " hooldust\u00F6\u00F6");
		add(headerPanel);
		...
		Button w = new Button("L\u00F5peta t\u00F6\u00F6!", new ClickHandler() {
		      public void onClick(ClickEvent event) {
		    	 // deviceTreeService.storeMaintenanceEntry(stp00.getValue(), stp11.getValue(), cp1.getValue(), null);
		    	  ...
		    	  mi.setMaintenanceDowntime(Integer.parseInt(stp00.getValue()));
		    	  mi.setMaintenanceTimeSpent(Integer.parseInt(stp11.getValue()));
		    	  mi.setMaintenanceCost(Double.parseDouble(cp1.getValue()));
		    	  mi.setMaintenanceCompleteDate(new Date());
		    	  deviceTreeService.storeMaintenanceEntry(mi, selectedCompany.getCompanyKey(), storeCallback);
